...
async def check_address(address: str, gaia_home: str = '/mnt/cosmos/rand'):
    """
    randd keys parse <address>
    """
    check = subprocess.run(["randd", "keys", "parse",
                            f"{address}",
                            f'--home={gaia_home}',
                            '--output=json'],
...
async def get_balance_list(address: str, node: str, gaia_home: str = '/mnt/cosmos/rand'):
    """
    randd query bank balances <address> <node> <chain-id>
    """
    balance = subprocess.run(["randd", "query", "bank", "balances",
                              f"{address}",
                              f'--node={node}',
                              f'--home={gaia_home}',
                              '--output=json'],
...
async def tx_send(request: dict):
    ...
    - "node"
    - "chain_id"
    randd tx bank send <from address> <to address> <amount>
                       <fees> <node> <chain-id>
                       --keyring-backend=test -y

    """
    tx_gaia = subprocess.run(['randd', 'tx', 'bank', 'send',
                              f'{request["sender"]}',
                              f'{request["recipient"]}',
                              f'{request["amount"]}',
                              f'--fees={request["fees"]}',
